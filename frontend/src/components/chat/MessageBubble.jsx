...
export default function MessageBubble({ role, text, createdAt }) {
  ...
  return (
    <div className={`max-w-[88%] border px-3 py-2 text-xs ${isUser ? "ml-auto border-cyan-500/40 bg-cyan-500/10" : "border-slate-700 bg-slate-900/70"}`}>
      <p className="whitespace-pre-wrap leading-relaxed text-slate-200">{text}</p>
      <p className="mt-1 text-[10px] uppercase tracking-[0.12em] text-slate-500">{formatTime(createdAt)}</p>
    </div>
  );
}
